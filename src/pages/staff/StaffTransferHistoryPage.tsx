...
export default function StaffTransferHistoryPage() {
  const { profile, user } = useAuth();

  const { data: transfers, isLoading } = useQuery({
    queryKey: ['staff-transfer-history', profile?.id],
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_transfers_with_emails');
      if (error) throw error;
      
      // Filter transfers where user is initiator or receiver
      return (data || []).filter((t: any) => 
        t.initiator_id === profile?.id || t.receiver_id === profile?.id
      );
    },
    enabled: !!profile?.id,
  ...
  return (
    <DashboardLayout
      ...
      pageSubtitle="View your item transfer history"
      notificationCount={0}
    >
      <Card>
        <CardHeader>
